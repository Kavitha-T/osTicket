...
switch(strtolower($_REQUEST['status'])){ //Status is overloaded
    case 'open':
        ...
        break;
	//changes to add waiting on dev
	case 'waitingondev':
        $status='open';
		$results_type=__('Waiting on dev');
        break;
	//changes to add waiting on cust	
	case 'waitingoncust':
        $status='open';
...
//changes to add waiting on dev and waiting on cust and to display only open-tickets in open status
 ...
 <table class="list" border="0" cellspacing="1" cellpadding="2" width="940">
    <thead>
        <tr>
	        ...
	        <th width="8px" class="hidden">&nbsp;</th>
            <?php } ?>
	        <th width="70" class="hidden">
            ...
            <tr id="<?php echo $row['ticket_id']; ?>">
                <?php if($thisstaff->canManageTickets()) {
                    ...
                    if($ids && in_array($row['ticket_id'], $ids))
                        $sel=true;
                    ?>
                <td align="center" class="nohover hidden">
                    <input class="ckb" type="checkbox" name="tids[]"
                        value="<?php echo $row['ticket_id']; ?>" <?php echo $sel?'checked="checked"':''; ?>>
                </td>
                <?php } ?>
                <td title="<?php echo $row['email']; ?>" nowrap class="hidden">
